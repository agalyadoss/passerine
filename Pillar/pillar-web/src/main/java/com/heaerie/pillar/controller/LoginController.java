...
import com.heaerie.pillar.storage.USER002TTMapper;
...
public class LoginController {
    ...
    public static class V2Login extends Controller<USER002TT, USER002TTMapper>{
        ...
        public void validate() throws PillarBadFormatException {
            ...
            if (Strings.isNullOrEmpty(user.getUsrId())) {
                throw new PillarBadFormatException("usr id is mandatory");
            }

            if (Strings.isNullOrEmpty(user.getPassword())) {
                throw new PillarBadFormatException("password is mandatory");
            }
        }
    ...
    private class V2Register extends Controller<USER002TT, USER002TTMapper> {
        ...
        public void validate() throws PillarBadFormatException {
            user = execute();
            if (Strings.isNullOrEmpty(user.getUsrId())) {
                throw new PillarBadFormatException("username is mandatory");
            }

            if (Strings.isNullOrEmpty(user.getDomain())) {
                throw new PillarBadFormatException("domain is mandatory");
            }
